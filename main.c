...
char * create_http_request_header(const char *get, char use_proxy_host, char get_instead_of_head, char persistent_connections, const char *hostname, const char *useragent, const char *referer, char ask_compression, char no_cache, const char *auth_usr, const char *auth_password, char **static_cookies, int n_static_cookies, char **dynamic_cookies, int n_dynamic_cookies, const char *proxy_buster, const char *proxy_user, const char *proxy_password, char **additional_headers, int n_additional_headers, char http2, char is_ssl)
{
	int index;
	char *request = NULL;
	char pb[128] = { 0 };
	...
	if (http2) {
		str_add(&request, "Connection: Upgrade, HTTP2-Settings\r\n");
		if (is_ssl) // https
			str_add(&request, "Upgrade: h2\r\n");
		else
			str_add(&request, "Upgrade: h2c\r\n");
...
persistent_loop:
			...
			if (http2 && get_instead_of_head) {
				printf("%s\n", reply);
				printf("%s\n", request);
				if (strncmp(reply, "HTTP/1.1 101", 12) != 0)
					printf("Server did not want to switch protocols\n");
				else if (strcasestr(reply, "Connection: Upgrade") == NULL)
					printf("Server did not want to upgrade connection\n");
				else if (strcasestr(reply, "Upgrade: h2c") == NULL)
					printf("Server did not want to upgrade connection to HTTP/2\n");
			}
